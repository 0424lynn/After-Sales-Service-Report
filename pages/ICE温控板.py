# -*- coding: utf-8 -*-
# ICE温控板配件分析（2.1.4.7）— 完整版（强制隐藏侧栏 + 兼容多版本）
...
# ================= 参数与数据读取 =================
code  = get_param_str("code", default="2.1.4.7")  # ← ICE温控板
...
st.caption(f"统计口径：{scope_text} | 缺陷码：{code}（Control board）")
if df_code.empty:
    st.warning("该口径下未命中任何 2.1.4.7（Control board）相关记录。")
    st.stop()
...
# ================= 2.1.4.7.x（Control board 子项）分项 =================
def build_2147_subtable(df_in: pd.DataFrame) -> pd.DataFrame:
    join = df_in.astype(str).agg(" ".join, axis=1).str.lower().str.replace("\u3000"," ", regex=False)
    join = join.str.replace(r"[\s\-_\/]+"," ", regex=True).str.strip()

    # 自动识别任何 2.1.4.7.X（X为一段数字/字母）的子项；若数据中没有子项，也会给出顶层
    subcode_matches = join.str.extractall(r"(?<![0-9a-z])(2\.1\.4\.7\.[0-9a-z]+)(?![0-9a-z])", flags=re.I)[0]
    ...
    for sub in unique_subs:
        ...
        rows.append({"编码": sub.upper(), "中文描述": "子项", "数量": qty, "费用": fee})

    # 顶层 2.1.4.7（不含任何 .7.x）
    p_top = re.compile(r"(?<![0-9a-z])2\.1\.4\.7(?![0-9a-z])", re.I)
    ...
    rows.insert(0, {"编码":"2.1.4.7","中文描述":"温控板（未细分）","数量":qty_top,"费用":fee_top})
...
# —— 渲染分项表
df_2147 = build_2147_subtable(df_code)
st.markdown("### 2.1.4.7.x 分项（Control board）")
if not df_2147.empty:
    show = df_2147.copy(); show["费用"] = show["费用"].map(lambda v: f"${float(v):,.0f}")
    st.dataframe(show, use_container_width=True)
    st.download_button(
        "下载 CSV（2.1.4.7 分项）",
        data=df_2147.to_csv(index=False, encoding="utf-8-sig"),
        file_name=f"ice_control_board_2147_breakdown_{scope_text.replace(' ','_')}.csv",
        mime="text/csv", use_container_width=True
    )
else:
    st.info("暂无 2.1.4.7 相关记录。")
...
# === 新增：具体型号 → 系列键（YR140/YR280/.../HD350）映射，用于“按具体型号”的在保量 ===
...
def build_tables(df_subset: pd.DataFrame, *, year=None, month=None):
    ...
    # 表2：按具体型号 —— 在保量改为：具体型号 → 系列键 → 取系列在保量
    tbl2=pd.DataFrame()
...
render_sortable_table("温控板（按型号系列）", (t1 if not t1.empty else base1),
    order_cols=("型号系列","短期（1-4月）","中期（5-8月）","长期（9-24月）","总计","在保量","问题比例(%)"))
render_sortable_table("温控板（按具体型号）", (t2 if not t2.empty else base2),
...
def _subset_2147_item(df_src: pd.DataFrame, sub_code: str)->pd.DataFrame:
    if df_src is None or df_src.empty: return df_src.iloc[0:0].copy()
    j=_clean_join(df_src)
    sub = (sub_code or "").strip()
    if not sub or not re.match(r"^2\.1\.4\.7\.[0-9a-z]+$", sub, re.I):
...
rows_need = (
    df_2147[
        df_2147["编码"].astype(str).str.match(r"^2\.1\.4\.7\.[0-9a-z]+$", na=False) &
        (pd.to_numeric(df_2147["数量"], errors="coerce").fillna(0) > 0)
    ]
...
if rows_need.empty:
    st.info("当前 2.1.4.7.x 没有数量>0 的条目可深入分析。")
else:
    for idx, r in rows_need.iterrows():
        sub_code = str(r["编码"])
        zh_name  = str(r.get("中文描述","子项"))
        sub_df = _subset_2147_item(df_code, sub_code)
...
# ================= 原始明细（命中 2.1.4.7 的记录） =================
with st.expander("原始明细（命中 2.1.4.7 的记录）", expanded=False):
    ...
    st.download_button(
        ...
        file_name="ice_control_board_2147_raw_details.csv",
        mime="text/csv",
