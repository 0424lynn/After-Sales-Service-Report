...
try:
    st.set_page_config(page_title="外部漏水分析（1.1.5.2）", layout="wide", initial_sidebar_state="collapsed")
except Exception:
    pass
...
# ================= 参数与数据读取 =================
code  = get_param_str("code", default="1.1.5.2")  # ← 外部漏水
...
if df_code.empty:
    st.warning("该口径下未命中任何 1.1.5.2（外部漏水）相关记录。")
    st.stop()
# ================= 顶部抬头条 =================
...
# ================= 1.1.5.2.x（外部漏水）分项 =================
def build_1152_subtable(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    # 只用“编码命中”，中文列名按你提供的翻译
    pats = {
        "1.1.5.2.1 Air Relief Valve": ("气压平衡阀",      _pat("1.1.5.2.1")),
        "1.1.5.2.2 Condensation waster on glass door surface/ mullion Condesation water": ("门表面冷凝水", _pat("1.1.5.2.2")),
        "1.1.5.2.4 backpack overflow": ("后背包溢水",     _pat("1.1.5.2.4")),
        "1.1.5.2.5 Wicking":           ("吸水纸",         _pat("1.1.5.2.5")),
        "1.1.5.2.6 Resecured insulation tape on the suction line": ("回气管漏水", _pat("1.1.5.2.6")),
        "1.1.5.2.7 back drain pan crack": ("后背包开裂",   _pat("1.1.5.2.7")),
        "1.1.5.2.8 The area around the tube is not sealed": ("回气管未打胶", _pat("1.1.5.2.8")),
    }
    ...
    rows.insert(0, {"英文名称":"1.1.5.2 External Water Leakage","质量问题":"外部漏水","数量":qty_top,"费用":fee_top})
...
# —— 渲染分项表
df_1152 = build_1152_subtable(df_code)
st.markdown("### 1.1.5.2.x 分项（外部漏水）")
if not df_1152.empty:
    show = df_1152.copy(); show["费用"] = show["费用"].map(lambda v: f"${float(v):,.0f}")
    st.dataframe(show, use_container_width=True)
    st.download_button("下载 CSV（1.1.5.2 分项）",
        data=df_1152.to_csv(index=False, encoding="utf-8-sig"),
        file_name=f"external_leak_1152_breakdown_{scope_text.replace(' ','_')}.csv",
        mime="text/csv", use_container_width=True)
else:
    st.info("暂无 1.1.5.2 相关记录。")

# ================= 三张统计表（按系列 / 按具体型号；外部漏水无需“部件/型号”汇总） =================
USE_MONTH_CAND = [
...
render_sortable_table("外部漏水（按系列）", (t1 if not t1.empty else base1),
    order_cols=("型号","短期（1-4月）","中期（5-8月）","长期（9-24月）","总计","在保量","问题比例(%)"))
render_sortable_table("外部漏水（按具体型号）", (t2 if not t2.empty else base2),
...
def _subset_1152_item(df_src: pd.DataFrame, en_name: str)->pd.DataFrame:
    if df_src is None or df_src.empty: return df_src.iloc[0:0].copy()
    j=_clean_join(df_src)
    m = re.search(r"(1\.1\.5\.2\.[1-8])", en_name or "")
    if m:
...
rows_need = (
    df_1152[
        df_1152["英文名称"].astype(str).str.match(r"^1\.1\.5\.2\.[1-8]\b", na=False) &
        (pd.to_numeric(df_1152["数量"], errors="coerce").fillna(0) > 0)
    ]
    .sort_values("数量", ascending=False)   # ← 这里
    .copy()
)


if rows_need.empty:
    st.info("当前 1.1.5.2.x 没有数量>0 的条目可深入分析。")
else:
    for idx, r in rows_need.iterrows():
        en_name = str(r["英文名称"]); zh_name = str(r.get("质量问题",""))
        sub_df = _subset_1152_item(df_code, en_name)
...
# ================= 原始明细（命中 1.1.5.2 的记录） =================
with st.expander("原始明细（命中 1.1.5.2 的记录）", expanded=False):
    ...
    st.download_button(
        ...
        file_name="external_leak_1152_raw_details.csv",
        mime="text/csv",
