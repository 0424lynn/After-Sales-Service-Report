# -*- coding: utf-8 -*-
# ICE调试厚冰配件分析（2.1.5.1）— 完整版（强制隐藏侧栏 + 兼容多版本）

# ================== 强制隐藏侧栏（更强兼容版） ==================
import streamlit as st
import streamlit.components.v1 as components

# 只保留一次 set_page_config，尽量放最前（若上层已设置会忽略）
try:
    st.set_page_config(
        page_title="ICE调试厚冰配件分析（2.1.5.1）",
        layout="wide",
...
# —— CSS：覆盖各种 Sidebar 容器、导航、折叠手柄、头部按钮（多版本 data-testid）
st.markdown(
    """
<style>
/* 侧栏本体与容器 */
...
# —— JS：页面动态变化时持续再隐藏（MutationObserver + interval）
components.html(
    """
<script>
(function(){
...
SPEC_FLEET_JSON   = Path("spec_fleet_counts_ice.json")    # 具体型号在保量（ICE，可选）
...
# ================= 参数与数据读取 =================
code  = get_param_str("code", default="2.1.5.1")  # ← 调试厚冰
...
st.caption(f"统计口径：{scope_text} | 缺陷码：{code}（Debug / Ice thickness）")
if df_code.empty:
    st.warning("该口径下未命中任何 2.1.5.1（调试厚冰）相关记录。")
    st.stop()
...
# ================= 2.1.5.1.x（调试厚冰 子项）分项 =================
def build_2151_subtable(df_in: pd.DataFrame) -> pd.DataFrame:
    join = df_in.astype(str).agg(" ".join, axis=1).str.lower().str.replace("\u3000"," ", regex=False)
    join = join.str.replace(r"[\s\-_\/]+"," ", regex=True).str.strip()

    # 自动识别任何 2.1.5.1.X（X为一段数字/字母）的子项；若数据中没有子项，也会给出顶层
    subcode_matches = join.str.extractall(r"(?<![0-9a-z])(2\.1\.5\.1\.[0-9a-z]+)(?![0-9a-z])", flags=re.I)[0]
    ...
    for sub in unique_subs:
        ...
        rows.append({"编码": sub.upper(), "中文描述": "探头", "数量": qty, "费用": fee})

    # 顶层 2.1.5.1（不含任何 .5.1.x）
    p_top = re.compile(r"(?<![0-9a-z])2\.1\.5\.1(?![0-9a-z])", re.I)
    ...
    rows.insert(0, {"编码":"2.1.5.1","中文描述":"调试厚冰（未细分）","数量":qty_top,"费用":fee_top})
...
# —— 渲染分项表
df_2151 = build_2151_subtable(df_code)
st.markdown("### 2.1.5.1.x 分项（Debug / Ice thickness）")
if not df_2151.empty:
    show = df_2151.copy(); show["费用"] = show["费用"].map(lambda v: f"${float(v):,.0f}")
    st.dataframe(show, use_container_width=True)
    st.download_button(
        "下载 CSV（2.1.5.1 分项）",
        data=df_2151.to_csv(index=False, encoding="utf-8-sig"),
        file_name=f"ice_ice_thickness_debug_2151_breakdown_{scope_text.replace(' ','_')}.csv",
        mime="text/csv", use_container_width=True
    )
else:
    st.info("暂无 2.1.5.1 相关记录。")
...
def spec_to_series_key(spec: str) -> str:
    """
    将“具体型号”文本映射到系列键（YR140/YR280/.../HD350）。
    规则：
    1) 直接在去符号的字符串里查找已知系列键；
    2) 否则尝试用正则从开头抽取（如 YR-280A → YR280）；
    3) 再否则回退到 resolve_model_key。
    """
    s_raw = (spec or "").upper()
    s = re.sub(r"[\s\-_/]", "", s_raw)

    # 先精确包含匹配（长度排序，避免短键误匹配）
    for k in sorted(ICE_MODEL_ORDER, key=len, reverse=True):
        if k.replace("-", "") in s:
            return k

    # 尝试从开头抽取 (YR|CYR|HD)+3位数字
...
def build_tables(df_subset: pd.DataFrame, *, year=None, month=None):
    ...
    if spec_col is not None:
        ...
        # —— 在保量按“具体型号 → 系列键（YR140...）”映射后，从系列在保量中取值
        fleet = load_model_fleet()
        month_fleet = get_fleet_month(fleet, year, month)

        def _spec_fleet_lookup(spec_name: str) -> int:
            series_key = spec_to_series_key(spec_name)   # 关键改动：先映射到系列键
            return int(month_fleet.get(series_key, 0))  # 再取该系列的在保量
...
render_sortable_table("调试厚冰（按型号系列）", (t1 if not t1.empty else base1),
    order_cols=("型号系列","短期（1-4月）","中期（5-8月）","长期（9-24月）","总计","在保量","问题比例(%)"))
render_sortable_table("调试厚冰（按具体型号）", (t2 if not t2.empty else base2),
...
def _subset_2151_item(df_src: pd.DataFrame, sub_code: str)->pd.DataFrame:
    if df_src is None or df_src.empty: return df_src.iloc[0:0].copy()
    j=_clean_join(df_src)
    sub = (sub_code or "").strip()
    if not sub or not re.match(r"^2\.1\.5\.1\.[0-9a-z]+$", sub, re.I):
...
rows_need = (
    df_2151[
        df_2151["编码"].astype(str).str.match(r"^2\.1\.5\.1\.[0-9a-z]+$", na=False) &
        (pd.to_numeric(df_2151["数量"], errors="coerce").fillna(0) > 0)
    ]
...
if rows_need.empty:
    st.info("当前 2.1.5.1.x 没有数量>0 的条目可深入分析。")
else:
    for idx, r in rows_need.iterrows():
        sub_code = str(r["编码"])
        zh_name  = str(r.get("中文描述","探头"))
        sub_df = _subset_2151_item(df_code, sub_code)
...
# ================= 原始明细（命中 2.1.5.1 的记录） =================
with st.expander("原始明细（命中 2.1.5.1 的记录）", expanded=False):
    ...
    # —— 与“原始 Excel 表映射（ICE）”关联：补【原始生产日期】【原始序列号】
    ...
    st.download_button(
        ...
        file_name="ice_ice_thickness_debug_2151_raw_details.csv",
        mime="text/csv",
